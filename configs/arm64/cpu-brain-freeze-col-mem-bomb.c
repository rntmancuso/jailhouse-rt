...
#define MAIN_PHYS_START       (0x0801200000 + BOMB_ID * 16 * MAIN_SIZE)
#define MAIN_PHYS_START_SCHIM (0x1001200000 + (BOMB_ID+1) * 16 * MAIN_SIZE) // 0x10_0000_0000 as dual ported SchIm is used
...
struct {
	...
	struct jailhouse_memory_colored col_mem[2];
} __attribute__((packed)) config = {
	.cell = {
		.signature = JAILHOUSE_CELL_DESC_SIGNATURE,
		.revision = JAILHOUSE_CONFIG_REVISION,
		.name = "cbf-col-mem-bomb-" xstr(BOMB_ID),
	...
	.col_mem = {
		...
		},
		{
			/* RAM through SchIM */
			.memory = {
				.phys_start = MAIN_PHYS_START_SCHIM,
				.virt_start = 0xa00000,
				.size = MAIN_SIZE,
				.flags = JAILHOUSE_MEM_READ | JAILHOUSE_MEM_WRITE |
					JAILHOUSE_MEM_EXECUTE | JAILHOUSE_MEM_LOADABLE,
